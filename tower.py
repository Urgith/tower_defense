...
class Tower:
    ...
    def __init__(self, mouse_pos, type, counter, dt):
        ...
        (self.image, self.color, self.full_price, self.damage, self.speed, self.range,
            self.reload, self.bullet_size) = WIEZE[self.type]
    ...
    def upgrade(self, game, upgrade_id):
        ...
        if (upgrade_id % 4) == 0 and (game.money >= TOWER_UPGRADES[self.type][0][0]) and (self.level_damage <= 4):
            self.increase_damage(TOWER_UPGRADES[self.type][0][1])
            self.increase_cost(game, TOWER_UPGRADES[self.type][0][0])

            if self.type == 2:
                self.multishot += 1

        elif (upgrade_id % 4) == 1 and (game.money >= TOWER_UPGRADES[self.type][1][0]) and (self.level_range <= 4):
                self.increase_range(TOWER_UPGRADES[self.type][1][1])
                self.increase_cost(game, TOWER_UPGRADES[self.type][1][0])
        ...
        self.image = WIEZE[self.type + (self.level * 3)][0]
    ...
    def increase_special(self, game, reload=30, pierce_rate=2, electro_rate=4):
        ...
        if (game.money >= TOWER_UPGRADES[self.type][2]):
            self.level_special += 1
            self.speed += 50
            self.increase_cost(game, TOWER_UPGRADES[self.type][2])
